...
#
#    "The network is trained to label the nodes and edges of the sconhortest path, given the start and end nodes.\n",
#
...
def generate_featured_graphs(rand,
    ...
    if dataset=="Willow":
        ...
        graphs, _, _, _ = VOC.gen_random_graphs_VOC(rand,
                                              ...
                                              use_train_set=use_train_set,
                                              category_id=category_id)
    elif dataset=="Spair71k":
        graphs, _, _, _ = Spair71k.gen_random_graphs_Spair17k(rand,
                                                    batch_size,
                                                    num_inner_min_max,
                                                    num_outlier_min_max,
                                                    feaType=visfeaType,
    ...
    elif dataset=="CUB_2011":
        graphs, _, _, _ = CUB_2011.gen_random_graphs_CUB(rand,
                                                    batch_size,
                                                    num_inner_min_max,
                                                    num_outlier_min_max,
                                                    feaType=visfeaType,
        ...
        graphs, _, _, _ = ICMPT.gen_random_graphs_ICMPT(rand,
                                                    batch_size,
                                                    num_inner_min_max,
                                                    num_outlier_min_max,
                                                    feaType=visfeaType,
    ...
    elif dataset == "QAPDATA":
        graphs, _, _, _ = qaplib.gen_random_graphs_QAP(rand,
                                                       batch_size,
                                                       use_train_set = use_train_set,
                                                       sample_id = sample_id)
...
def create_placeholders(rand,
                        ...
                        num_inner_min_max,
                        num_outlier_min_max,
                        visfeaType = "",dataset = "",sample_id = -1):
  ...
  input_graphs, target_graphs, _ = generate_featured_graphs(
      rand, batch_size, num_inner_min_max, num_outlier_min_max, visfeaType = visfeaType,
...
def create_feed_dict(rand,
  ...
  inputs, targets, raw_graphs = generate_featured_graphs(
      ...
      visfeaType = visfeaType, use_train_set = use_train_set,dataset=dataset, category_id= category,
  sample_id= sample_id)
  input_graphs = utils_featured_graph.featured_graphs_to_graphs_tuple(inputs)
  target_graphs = utils_featured_graph.featured_graphs_to_graphs_tuple(targets)
...
def softmax(x):
    ...
    new_x = x - np.transpose(np.vstack([max_x, max_x]))

    totalSum = np.sum(np.exp(new_x), axis = 1)
    totalSum = np.transpose(np.vstack([totalSum,totalSum]))
    np.seterr(divide='ignore', invalid='ignore')
    soft_x = np.exp(new_x)/totalSum
    return soft_x

def compute_accuracy_hungurian(target, output, use_hungarian = True):
  ...
  for td, od in zip(tdds, odds):
      rows = np.max(od["group_indices_1"]) + 1
      cols = np.max(od["group_indices_2"]) + 1

      if NODE_OUTPUT_SIZE == 1:
          tlabels = np.round(td["nodes"])
       #   olabels = np.round(od["edges_ex"])
       #   olabels = greedy_mapping(od["edges_ex"], od["senders_ex"], od["receivers_ex"])
       #   olabels = hungarian(od["edges_ex"], od["senders_ex"], od["receivers_ex"])
          vals = od["nodes"]
      else:
          tlabels = np.round(td["nodes"][:, 1])
          nodes = od["nodes"]
      #    olabels = greedy_mapping(edges_ex[:, 1], od["senders_ex"], od["receivers_ex"])
      #   olabels = hungarian(edges_ex[:, 1], od["senders_ex"], od["receivers_ex"])
          vals = nodes[:, 1]

      X = np.zeros(shape = (rows, cols), dtype = vals.dtype)
      gX = np.zeros(shape = (rows, cols), dtype = vals.dtype)
      for i in range(len(vals)):
          X[od["group_indices_1"][i]][od["group_indices_2"][i]] = vals[i]
          gX[od["group_indices_1"][i]][od["group_indices_2"][i]] = tlabels[i]
      ...
      for i in range(len(vals)):
          vals[i] = X[od["group_indices_1"][i]][od["group_indices_2"][i]]
      olabels = vals.astype(tlabels.dtype)

      c_all = np.mean((tlabels == olabels))
      gt_idx = np.where(tlabels)

      c_gt = np.mean((tlabels[gt_idx] == olabels[gt_idx]))
      acc = np.sum((tlabels[gt_idx] == olabels[gt_idx])) / td["globals"][0]
      ...
      solved = solved + np.prod(c_all)


  correct_all = np.mean(cs_all)
  correct_gt = np.mean(cs_gt)
#  correct_solved = solved / len(tdds)
  accuracy = np.mean(accuracy)

  return correct_all, correct_gt, accuracy, Xs, gXs
...
def create_loss_ops(target_op, output_ops, loss_cof):
    ...
    if NODE_OUTPUT_SIZE == 1: 
        ...
        # loss_nodes_1 = tf.losses.softmax_cross_entropy(label_cof * target_op.nodes, label_cof * (output_op.nodes+1e-6))
        # loss_nodes_2 = tf.losses.softmax_cross_entropy((1 - target_op.nodes), (1 - output_op.nodes))
        # loss_ops = (loss_nodes_1 + loss_nodes_2)/2.0
        loss_nodes=label_cof*(target_nodes*tf.log(output_nodes+(1e-6)))+(1-target_nodes)*tf.log(1-output_nodes+(1e-6))
        loss_ops=(-tf.reduce_mean(loss_nodes))
    else:
...
#     target_nodes=target_op.nodes
#     output_nodes=output_op.nodes
#     group_indices_1 = target_op.group_indices_1
#     group_indices_2 = target_op.group_indices_2
#     n_batch = target_op.n_batch
#
#     sum_kpts = tf.cast(tf.reduce_sum(target_op.n_group_1), tf.int64)
#
#     positive_prediction = target_nodes * output_nodes
#
#     pp_row = tf.unsorted_segment_sum(positive_prediction, group_indices_1, sum_kpts)
#     pp_row_mat = tf.gather(pp_row, group_indices_1) - 0.1
#     mask_neg_rowmat = tf.greater(output_nodes, pp_row_mat)
#     mask_neg_rowmat = tf.cast(mask_neg_rowmat, tf.float64)
#
#     neg_valid = mask_neg_rowmat * tf.pow(output_nodes, 2)
#     neg_valid_rowsum = tf.unsorted_segment_sum(neg_valid, group_indices_1, sum_kpts) + 1.0
#     loss_row = (-0.5 * tf.log((tf.pow(pp_row,2) + 1e-6)/neg_valid_rowsum))
#     loss_row = tf.unsorted_segment_sum(loss_row, target_op.n_batch, tf.reduce_max(n_batch) + 1) / tf.cast(target_op.n_group_1, tf.float64)
#     loss_row = tf.reduce_mean(loss_row)
#
#     pp_col = tf.unsorted_segment_sum(positive_prediction, group_indices_2, sum_kpts)
#     pp_col_mat = tf.gather(pp_col, group_indices_2) - 0.1
#     mask_neg_colmat = tf.greater(output_nodes, pp_col_mat)
#     mask_neg_colmat = tf.cast(mask_neg_colmat, tf.float64)
#
#     neg_valid = mask_neg_colmat * tf.pow(output_nodes, 2)
#     neg_valid_colsum = tf.unsorted_segment_sum(neg_valid, group_indices_2, sum_kpts) + 1.0
#     loss_col = (-0.5 * tf.log((tf.pow(pp_col,2) + 1e-6) / neg_valid_colsum))
#     loss_col = tf.unsorted_segment_sum(loss_col, target_op.n_batch, tf.reduce_max(n_batch) + 1) / tf.cast(target_op.n_group_2, tf.float64)
#     loss_col = tf.reduce_mean(loss_col)
#
#     loss_ops = (loss_row + loss_col) *0.5
#
#     return loss_ops
