...
def load_trained_model(model_file,num_conv):
...
def evaluate_VOC(sess,
    ...
    for i in range(num_categories):
        ...
        for j in range(num_batch):
            ...
            # correct_gt_ge, correct_all_ge, solved_ge, matches_ge, Xs, gXs = gmc.compute_accuracy(
            #     eval_values["target"], eval_values["outputs"][-1], use_edges=False)
            correct_all_ge, correct_gt_ge, matches_ge, Xs, gXs = gmc.compute_accuracy_hungurian(eval_values["target"], eval_values["outputs"][-1])
            # print("{}, {}, {:.4f}".format(i, j, correct_gt_ge))
...
for num_conv in range(10):
    model_file = "trained_models/new_sinkhorn_pos_w1_Pascal_EKpb-0.7_DKpb-0.7_CKpb-1.0_MPI-2_LD-128_LR-0.003_GM-DEL_RR-0.0-BBGM/best_model_acc-87000"
    sess, input_ph, target_ph, loss_cof_ph, keep_prob_encoder_ph,keep_prob_decoder_ph,kepp_prob_conv_ph,output_ops_ge= load_trained_model(model_file,num_conv+1)
