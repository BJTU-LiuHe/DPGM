...
assert GM_GenData.DATASET == "Pascal"
...
def train_proc(queue):
    ...
    learning_rate = tf.maximum(0.00025, tf.train.exponential_decay(lr, global_step, 1000, 0.995, staircase = False))
    ...
    # #load trained model
    if reload:
        model_file = os.path.join(path_reload, "best_model_acc-360000")
    ...
    for iteration in range(last_iteration, num_training_iterations):
        ...
        if iteration % 100 ==0:
            ...
            if np.mean(np.array(correct_gt_tr_list))> 0.80:
                start_eval = True

            losses_tr.clear()
            correct_all_tr_list.clear()
            correct_gt_tr_list.clear()
        if (iteration + 1) % 1000 == 0 and start_eval:
            last_time = the_time
            accuracy_dict = {}
            ...
            accuracy_avg, accuracy_avg_var = np.mean(accuracy_avg).item(), np.var(accuracy_avg).item()
            ...
            if accuracy_avg > max_acc:
                ...
                for key, value in accuracy_dict.items():
                    logger.info(key + ":" + str(np.mean(value).item()) + "---" + str(np.var(value)))

                logger.info("avg:" + str(accuracy_avg) + "---" + str(accuracy_avg_var))
