...
def _gen_features_VOC(pts0, tails0, heads0,pts_fea0, patches0, pts1, tails1, heads1,pts_fea1, patches1):
    ...
    num_matches = num_nodes0 * 3
    dict_matches = dict()

    id_match = 0
    for idx in range(num_nodes0):
        feature0 = pts_fea0[idx]
        simi_curr = np.matmul(feature0, pts_fea1)
        sorted_indices = np.argsort(simi_curr)[::-1]
        for ind in range(topk):
            dict_matches[id_match] = {"gidx1": idx, "gidx2": ind}
            id_match += 1
