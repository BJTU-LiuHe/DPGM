...
class InteractionNetwork(snt.AbstractModule):#snt.AbstractModule
...
class GraphNetwork(snt.AbstractModule):
  ...
  def _build(self, graph,keep_prob):
    ...
    """
    return self._global_block(self._node_block(self._edge_block(graph,keep_prob),keep_prob),keep_prob)
...
class GraphIndependent(snt.AbstractModule):
  ...
  def _build(self, graph,keepprob):
    ...
    """
    nodes = tf.nn.dropout(self._node_model(graph.nodes),keep_prob=keepprob)
    edges = tf.nn.dropout(self._edge_model(graph.edges),keep_prob=keepprob)
    globals = tf.nn.dropout(self._global_model(graph.globals),keep_prob=keepprob)
...
class GraphIndependentReLU(snt.AbstractModule):
  ...
  def my_sinkhorn(self, C,graphs, niter=1):
    sum_kpts = tf.cast(tf.reduce_sum(graphs.n_node), tf.int64)
    P=C
    for _ in range(niter):
        norm_row = 1.0/(tf.unsorted_segment_sum(P, graphs.group_indices_1, sum_kpts) + 1e-8)
        norm_row = tf.gather(norm_row, graphs.group_indices_1)
        P=P*norm_row

        norm_col= 1.0/ (tf.unsorted_segment_sum(P, graphs.group_indices_2, sum_kpts) + 1e-8)
        norm_col= tf.gather(norm_col, graphs.group_indices_2)
        P=P*norm_col

    return P

  def _prob_matching(self,graphs,edges,nodes):

    # def my_sinkhorn(C, graphs, f=None, niter=1):
    #
    #   sum_kpts = tf.cast(tf.reduce_sum(graphs.n_node), tf.int64)
    #   # C = graphs.nodes
    #   if f is None:
    ...
    #       exp_fc = tf.exp((-f - C))
    #       groups_1 = tf.unsorted_segment_sum(exp_fc, graphs.group_indices_1, sum_kpts)
    #       groups_1 = tf.log(groups_1)
    #       g = tf.gather(groups_1, graphs.group_indices_1)
    #
    #       exp_gc = tf.exp(-g - C)
    #       groups_2 = tf.unsorted_segment_sum(exp_gc, graphs.group_indices_2, sum_kpts)
    #       groups_2 = tf.log(groups_2)
    #       f = tf.gather(groups_2, graphs.group_indices_2)
    #
    #   P = tf.exp(-f - g - C)
    #
    #   return P
    ...
    iteration=10
    sum_kpts = tf.cast(tf.reduce_sum(graphs.n_node), tf.int64)
    # index_K_perm=tf.concat((tf.expand_dims(graphs.receivers,axis=1),
    #                    tf.expand_dims(graphs.senders,axis=1)),axis=1)
    index_K = tf.concat((tf.expand_dims(graphs.senders, axis=1),
                         tf.expand_dims(graphs.receivers, axis=1)), axis=1)
    index_K=tf.cast(index_K,dtype=tf.int64)
    # index_K_perm =tf.cast(index_K_perm,dtype=tf.int64)
    size_K=tf.cast(tf.reduce_sum(graphs.n_node),tf.int64)

    # size_K_f=tf.cast(tf.reduce_sum(graphs.n_node),tf.float64)
    value_K=tf.squeeze(edges,axis=1)
    Ks=tf.SparseTensor(indices=index_K, values=value_K, dense_shape=[size_K, size_K])
    # Ks_perm = tf.SparseTensor(indices=index_K_perm, values=value_K, dense_shape=[size_K, size_K])

    # Ks = tf.sparse_to_dense(sparse_indices=index_K, sparse_values=value_K, output_shape=[size_K, size_K])
    # Ks_perm=tf.sparse_to_dense(sparse_indices=index_K_perm,output_shape=[size_K,size_K],sparse_values=value_K)

    # Ks= tf.sparse_add(Ks,Ks_perm)
    ...
    for ite in range(iteration):
      ...
      Ps = tf.cond(tf.norm(Ps-Ps_last)>0.000001, lambda : Ps, lambda : Ps_last)

    # Ps = self.my_sinkhorn(Ps, graphs)
  ...
  def _build(self, graph):
    ...
    """
    nodes = tf.nn.sigmoid(self._node_model(graph.nodes))
    edges = tf.nn.sigmoid(self._edge_model(graph.edges))
    globals = tf.nn.relu(self._global_model(graph.globals))
    nodes = self._prob_matching(graph,edges,nodes)
    return graph.replace(
        nodes=nodes,
        edges=edges,
        globals=globals
...
class GraphGroupAggregator(snt.AbstractModule):
  ...
  def _build(self, graph):
    ...
    groups_1 = tf.unsorted_segment_sum(graph.nodes, group_indices_1, num_groups_1)
    groups_2 = tf.unsorted_segment_sum(graph.nodes, group_indices_2, num_groups_2)
