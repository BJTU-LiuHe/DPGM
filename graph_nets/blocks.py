...
class EdgeBlock(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    sender_size = 2 * models.LATENT_SIZE #sender_feature.shape.as_list()[1]
    self._metric1 = tf.Variable(tf.random_normal(shape=[sender_size, sender_size], dtype = tf.float64, mean=0, stddev=1),
        dtype = tf.float64,
        name='edge_metric1')
    receiver_size = 2 * models.LATENT_SIZE #receiver_feature.shape.as_list()[1]
    self._metric2 = tf.Variable(tf.random_normal(shape=[receiver_size, receiver_size], dtype=tf.float64, mean=0, stddev=1),
        dtype=tf.float64,
        name='edge_metric2')
  ...
  def _build(self, graph,keep_prob):
    ...
    # sum
    # edges_to_collect.append(sender_feature + receiver_feature)

    # # max
    # features = tf.stack([sender_feature, receiver_feature])
    # max_features = tf.reduce_max(features, axis = 0)
    # edges_to_collect.append(max_features)

    # # min
    # features = tf.stack([sender_feature, receiver_feature])
    # min_features = tf.reduce_min(features, axis = 0)
    # edges_to_collect.append(min_features)

    # # concatenate
    # edges_to_collect.append(sender_feature)
    # edges_to_collect.append(receiver_feature)
  ...
  #  updated_edges = self._edge_model(collected_edges)
    ...
    updated_edges = tf.nn.dropout(updated_edges,keep_prob=keep_prob)
...
class NodeBlock(snt.AbstractModule):
  ...
  #  updated_nodes = tf.nn.leaky_relu(self._node_model(collected_nodes))
    ...
    updated_nodes = tf.nn.dropout(updated_nodes,keep_prob=keep_prob)
...
class GlobalBlock(snt.AbstractModule):
  ...
  #  updated_globals = tf.nn.leaky_relu(self._global_model(collected_globals))
    ...
    updated_globals = tf.nn.dropout(updated_globals,keep_prob=keep_prob)
