...
class GraphNetwork(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    # print(edge_block_opt)
    # print(node_block_opt)
    # print(group_block_opt)
    # print(global_block_opt)
    # input()
    with self._enter_variable_scope():
      self._edge_block = blocks.EdgeBlock(
...
class GraphIndependentReLU(snt.AbstractModule):
  ...
  def my_sinkhorn(self, C,graphs, niter=5):
    sum_kpts = tf.cast(tf.reduce_sum(graphs.n_group_1), tf.int64)
    P=C
  ...
  def _prob_matching(self,graphs,edges,nodes):
    ...
    for ite in range(iteration):
      Ps_last = Ps
      Qs = Ks*tf.gather(Ps, graphs.receivers)
      Qs = tf.unsorted_segment_sum(Qs, graphs.senders, size_K)
      Ps = self.my_sinkhorn(Qs, graphs)
      delta= Ps / (Ps_last + 10e-6)
      Ks = Ks * (tf.gather(delta,graphs.senders))
      Ps = tf.cond(tf.norm(Ps-Ps_last)>0.000001, lambda : Ps, lambda : Ps_last)

    return Ps

  def _pure_sinkhron(self,graphs,edges,nodes):

    iteration=2
    Ks=edges
    Ps = self.my_sinkhorn(nodes, graphs)
  ...
  def _build(self, graph):
    ...
    nodes = self._pure_sinkhron(graph,edges,nodes)
    return graph.replace(
        nodes=nodes,
        edges=edges,
        globals=globals
