...
KEEPPROB_DECODER = GM_GenData.KEEPPROB_DECODER_ICMPT
KEEPPROB_CONV=GM_GenData.KEEPPROB_CONV_ICMPT
MEAN_POOLING_INT = GM_GenData.MEAN_POOLING_INTERVAL_ICMPT
LATENT_DIM=GM_GenData.LATENT_DIM_ICMPT
REGULAR_RATE = GM_GenData.REGULAR_RATE_ICMPT
if GM_GenData.GRAPH_MODE_ICMPT=="DEL":
    GRAPH_MODE="DEL"
elif GM_GenData.GRAPH_MODE_ICMPT=="KNN":
    GRAPH_MODE=str(GM_GenData.NUM_K_ICMPT)+"NN"

SAVE_ROOT= "trained_models_R1_topk/"+GM_GenData.DATASET+"_EKpb-"+str(KEEPPROB_ENCODER)+"_DKpb-"+str(KEEPPROB_DECODER)+"_CKpb-"+str(KEEPPROB_CONV)\
...
def train_proc(queue):
    ...
    model = models.EncodeProcessDecode(dynamic_core_num= 0,
                                       edge_output_size=1,
