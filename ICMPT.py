...
CLS_IMAGES_TRAIN = GM_GenData._map_cls_to_images(TRAIN_DATA_ROOT, CATEGORIES_TRAIN)
CLS_IMAGES_TEST = GM_GenData._map_cls_to_images(TEST_DATA_ROOT, CATEGORIES_TEST)
...
def gen_random_graphs_ICMPT(rand,
                              num_examples,
                              num_inner_min_max,
                              num_outlier_min_max,
                              feaType,
    ...
    for _ in range(num_examples):
        if category_id < 0:
            if use_train_set:
                cid = rand.randint(0, len(CATEGORIES_TRAIN))
            else:
                cid = rand.randint(0, len(CATEGORIES_TEST))
        else:
            cid = category_id

        if use_train_set:
            graph, image, matchInfo, affinity  = _gen_random_graph(rand, lib,
                                            CATEGORIES_TRAIN[cid],
