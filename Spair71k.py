...
Spair_ANNOTATION_ROOT = ROOT + "SPair71k/ImageAnnotation"
# VOC_FEAUTURES_ROOT = "data/PascalVoc/annotations_features"
Spair_BBGM_FEATURE_ROOT = ROOT + "BBGM_features/SPair71k"
Spair_IMAGE_ROOT = ROOT + "SPair71k/JPEGImages"
Spair_PAIRANNOTATION_ROOT= ROOT + "SPair71k/PairAnnotation"
...
train_dataset_dict={}
test_dataset_dict={}
train_anno_txt= ROOT + "SPair71k/Layout/large/trn.txt"
test_anno_txt= ROOT + "SPair71k/Layout/large/test.txt"
...
def _load_annotation(file) :
    ...
    h = float(json_data['image_height'])
    w = float(json_data['image_width'])
    boundbox = json_data['bndbox']
    ...
    for key,value in json_data["kps"].items():
        if value is not None:
            x = (value[0] - boundbox[0]) * 256 / (boundbox[2] - boundbox[0])
            y = (value[1] - boundbox[1]) * 256 / (boundbox[3] - boundbox[1])
            annoName.append(key)
            annoPts.append([x,y])
...
def _gen_features_SPAIR(pts0, tails0, heads0,pts_fea0, patches0, pts1, tails1, heads1,pts_fea1, patches1, use_train_set):
...
def _gen_random_graph(rand,
    ...
    while True:
        ...
        if use_train_set:
            ...
            pair_names = test_dataset_dict[category][sample_id]
        ...
        if anno_pts0.shape[0] < 3 or anno_pts1.shape[0] < 3:
            sample_id = random.randint(0, len(test_dataset_dict[category]) - 1)
            continue
