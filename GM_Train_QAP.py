...
def train_proc():
    ...
    learning_rate = tf.maximum(0.0001, tf.train.exponential_decay(lr, global_step, len(data_list) * 10, 0.99, staircase = False))
    ...
    for epoch in range(num_training_epoches):
        ...
        for iteration in range(len(data_list)):
            #print(data_list[sample_index[iteration]])
            ...
            test_sample_info = torch.load(
                os.path.join(qaplib.ROOT_PROCESSED_DATA, data_list[sample_index[iteration]] + "_dict.pth"))
            ...
            cur_result[data_list[sample_index[iteration]]] = res
            if res < baseline_res_dict[data_list[sample_index[iteration]]]:
                cur_count += 1
            # logger.info("# {:05d}, FT {:.1f}, TT {:.1f},  Ltr {:.4f}, CAtr {:.4f}, LR {:.5f}, current_result {:06d}, best_result {:06d}".format(
            #     epoch, feed_dict_time, training_time, np.mean(np.array(losses_tr)),
            #     np.mean(np.array(correct_all_tr_list)), train_values["learning_rate"], res, best_result))
        ...
        result_cur_epoch = []
        for name, res in cur_result.items():
            result_cur_epoch.append(res)
        results_all_epoches.append(result_cur_epoch)
        # if cur_count >=1:
        ...
        save_dict = {"results": results_all_epoches,
                     "names": data_list}

        torch.save(save_dict, os.path.join(SAVE_ROOT, "results_all.pth"))
